...
def process_account_order(db: Session, user_id: int=0, country_id: int=0, merchant_id: int=0, amount: float=0):
    ...
    da = debit_account(db=db, account_id=primary_account.id, amount=amount)
    if da['status'] == False:
        return {
            'status': False,
            'message': da['message'],
    ...
    create_transaction(db=db, country_id=country_id, currency_id=currency_id, gl_id=order_suspense_gl.id, type_id=order_trans_debit.id, action=TRANSACTION_ACTIONS['credit'], reference=reference, amount=amount, previous_balance=gl_prev_balance, new_balance=cgl['data']['balance'], status=1)
    return {
        'status': True,
        'message': 'Success',
        'data': main_trans
