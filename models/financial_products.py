...
def update_financial_product(db: Session, id: int=0, values: Dict={}):
    values['updated_at'] = get_laravel_datetime()
    db.query(FinancialProduct).filter_by(id = id).update(values)
    db.flush()
    return True

def delete_financial_product(db: Session, id: int=0):
    values = {
...
def force_delete_financial_product(db: Session, id: int=0):
