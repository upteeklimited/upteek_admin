...
def create_merchant_industry(db: Session, name: str = None, description: str = None, status: int = 0):
...
def update_merchant_industry(db: Session, id: int=0, values: Dict={}):
    values['updated_at'] = get_laravel_datetime()
    db.query(MerchantIndustry).filter_by(id = id).update(values)
    db.flush()
    return True

def delete_merchant_industry(db: Session, id: int=0):
    values = {
...
def force_delete_merchant_industry(db: Session, id: int=0):
