...
def create_tag(db: Session, merchant_id: int = 0, name: str = None, description: str = None, status: str = None, created_by: int = 0, commit: bool=False):
    tag = Tag(merchant_id=merchant_id, name=name, description=description, status=status, created_by=created_by, created_at=get_laravel_datetime(), updated_at=get_laravel_datetime())
    db.add(tag)
    if commit == False:
        db.flush()
    else:
