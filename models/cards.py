...
def create_card(db: Session, user_id: int = 0, provider: str = None, external_reference: str = None, external_email: str = None, card_type: str = None, card_bank: str = None, last_four_digits: str = None, card_expiry: str = None, meta_data: str = None, status: int = 0, deleted_at: str = None):
...
def update_card(db: Session, id: int=0, values: Dict={}):
    values['updated_at'] = get_laravel_datetime()
    db.query(Card).filter_by(id = id).update(values)
    db.flush()
    return True

def delete_card(db: Session, id: int=0):
    values = {
...
def force_delete_card(db: Session, id: int=0):
