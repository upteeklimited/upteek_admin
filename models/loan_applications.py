...
def update_loan_application(db: Session, id: int=0, values: Dict={}):
    values['updated_at'] = get_laravel_datetime()
    db.query(LoanApplication).filter_by(id = id).update(values)
    db.flush()
    return True

def delete_loan_application(db: Session, id: int=0):
    values = {
...
def force_delete_loan_application(db: Session, id: int=0):
