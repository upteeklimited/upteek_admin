...
def create_system_configuration(db: Session, name: str = None, single_value: str = None, multi_value: str = None):
...
def update_system_configuration(db: Session, id: int=0, values: Dict={}):
    values['updated_at'] = get_laravel_datetime()
    db.query(SystemConfiguration).filter_by(id = id).update(values)
    db.flush()
    return True

def delete_system_configuration(db: Session, id: int=0):
    values = {
...
def force_delete_system_configuration(db: Session, id: int=0):
