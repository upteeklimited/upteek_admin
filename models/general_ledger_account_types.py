...
def create_general_ledger_account_type(db: Session, country_id: int = 0, currency_id: int = 0, name: str = None, description: str = None, account_code: str = None, type_number: int = 0, status: int = 0, created_by: int = 0, authorized_by: int = 0, authorized_at: str = None):
...
def update_general_ledger_account_type(db: Session, id: int=0, values: Dict={}):
    values['updated_at'] = get_laravel_datetime()
    db.query(GeneralLedgerAccountType).filter_by(id = id).update(values)
    db.flush()
    return True

def delete_general_ledger_account_type(db: Session, id: int=0):
    values = {
...
def force_delete_general_ledger_account_type(db: Session, id: int=0):
