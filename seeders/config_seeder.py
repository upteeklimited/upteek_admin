...
def run_config_seeder(db: Session, commit: bool=False):
    ...
    create_system_configuration(db=db, name="vat_amount", single_value="7.5", commit=commit)
    create_system_configuration(db=db, name="whl_amount", single_value="7.5", commit=commit)
    gen_rev_account_code = "41090000"
    merchant_sales_commission_resp = create_gl(db=db, account_type_code=gen_rev_account_code, account_name="Merchant Sales Commission Account", 
    created_by=1, authorized_by=1, commit=commit)
    if merchant_sales_commission_resp['status'] == True:
        merchant_sales_commission_acct = merchant_sales_commission_resp['data']
        create_system_configuration(db=db, name="merchant_sales_commission_account", single_value=merchant_sales_commission_acct.account_number, commit=commit)
    customer_purchase_commission_resp = create_gl(db=db, account_type_code=gen_rev_account_code, account_name="Customer Purchase Commission Account", 
    created_by=1, authorized_by=1, commit=commit)
    if customer_purchase_commission_resp['status'] == True:
        customer_purchase_commission_acct = customer_purchase_commission_resp['data']
        create_system_configuration(db=db, name="customer_purchase_commission_account", single_value=customer_purchase_commission_acct.account_number, commit=commit)
    return True